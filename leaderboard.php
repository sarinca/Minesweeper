                ...
                <div style="display: flex; flex-direction: row;">
                    <select id="modeSelect" name="modeSelect" class="form-select m-2">
                        <?php foreach ($game_mode_filter as $game_mode): ?>
                            <option value=<?php echo $game_mode['mode'] ?>     <?php if ($selected_mode == $game_mode['mode'])
                                        echo 'selected'; ?>>
                    ...
                    <select id="friendSelect" name="friendSelect" class="form-select m-2">
                        <?php foreach ($friend_filter as $friend_mode): ?>
                            <option value=<?php echo $friend_mode['mode'] ?>     <?php if ($selected_users == $friend_mode['mode'])
                                        echo 'selected'; ?>>
                    ...
                    <div class="container m-2">
                        <div class="dropdown">
                            <button class="btn btn-light form-select" id="dropdownMenuButton" data-bs-toggle="dropdown"
                                <?php if ($selected_mode == "allPoints") {
                                    echo "disabled";
                                } ?>>
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <div class="slider-container p-2">
                                    <input type="range" min="0" max="<?php echo $slider_max; ?>"
                                        value="<?php echo $selected_time; ?>" class="slider" name="myRange"
                                        id="myRange">
                                    <p>Completion times faster than: <span id="sliderValue"
                                            style='font-size: 25px;'><?php echo $selected_time; ?></span> seconds</p>
                                </div>
                            </ul>
                        </div>
                    </div>
                ...
                <table class="table mt-3" style="width:90%">
                    <thead style="--bs-table-bg:rgba(249, 215, 143); border-color:rgba(249, 215, 143)">
                        <tr>
                            ...
                            <?php if ($selected_mode == "allPoints") {
                                echo "<th width='40%'><b>Total Points</b></th>";
                            } ?>
                            <?php if ($selected_mode != "allPoints") {
                                echo "<th width='40%'><b>Mode</b></th>";
                            } ?>
                            <?php if ($selected_mode != "allPoints") {
                                echo "<th width='40%'><b>Finish Time</b></th>";
                            } ?>
                        </tr>
                    </thead>
                    ...
                    <?php foreach ($leaderboard_entries as $board_entry):
                        ...
                        <tr class="<?php echo $row_class; ?>">
                            ...
                            <?php if ($selected_mode == "allPoints") {
                                echo "<td>" . $board_entry['totalScore'] . "</td>";
                            } ?>
                            <?php if ($selected_mode != "allPoints") {
                                echo "<td>" . $board_entry['mode'] . "</td>";
                            } ?>
                            <?php if ($selected_mode != "allPoints") {
                                echo "<td>" . $board_entry['gameTime'] . "</td>";
                            } ?>
                        </tr>
                    <?php endforeach; ?>
                </table>
